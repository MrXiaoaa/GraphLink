...
class Prompts:
    ...
    def get_prompt_dialect_sql_safety(self, api):
        if api in ["bigquery", "snowflake"]:
            return (
                "SQL dialect safety hints:\n"
                "- Do not nest analytic/window functions. For example, do not put LAG(), LEAD(), ROW_NUMBER(), RANK(), or SUM() OVER (...) inside another window function's ORDER BY, PARTITION BY, or aggregate expression. Compute the first window result in a CTE/subquery, then rank or aggregate in the next CTE.\n"
                "- Do not reference a SELECT alias or a window-function alias in the same SELECT's WHERE clause. Wrap it in a CTE/subquery first, or use QUALIFY only for filtering window-function results when the dialect supports it.\n"
                "- When calculating daily deltas, first compute the delta with LAG() in one CTE, then apply ROW_NUMBER()/RANK() over that delta in a later CTE.\n"
                "- Use only table and column names shown in the provided schema; keep fully qualified table names exactly as provided.\n"
            )
        if api == "sqlite":
            return (
                "SQL dialect safety hints:\n"
                "- SQLite does not support QUALIFY. If you need to filter a window-function result, compute it in a CTE/subquery and filter in the outer query.\n"
                "- Do not reference a SELECT alias in the same SELECT's WHERE clause; wrap it in a CTE/subquery first.\n"
                "- Use only table and column names shown in the provided schema.\n"
            )
        return ""
    ...
    def get_exploration_prompt(self, api, table_struct):
        ...
        exploration_prompt += self.get_prompt_dialect_sql_safety(api)
    ...
    def get_self_refine_prompt(self, table_info, task, pre_info, question, api, format_csv, table_struct, omnisql_format_pth=None):
        ...
        refine_prompt += self.get_prompt_dialect_sql_safety(api)
