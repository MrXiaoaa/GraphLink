    ...
    for func in eval_func:
        ...
        for ex in tqdm(os.listdir(pth)):
            ...
            for file in os.listdir(ex_pth):
                ...
                # 识别最终结果文件：result.csv 或 数字+result.csv (如 0result.csv, 1result.csv)
                is_final_result = False
                if file == "result.csv":
                    is_final_result = True
                elif re.match(r'^\d+result\.csv$', file):
                    is_final_result = True
                
                if file_pth.endswith(".csv"):
                    if is_final_result:
                        final_result_files.append(file_pth)
                    else:
                        # 其他 CSV 文件用于 pass@k 评估
                        ex_score.append(func(gold_result_dir, file_pth, ex, task=task))
            
            # 对于最终结果，选择所有候选中的最高分
            if final_result_files:
                scores = [func(gold_result_dir, f, ex, task=task) for f in final_result_files]
                final_score[ex] = max(scores)  # 取最高分作为最终得分
        ...
        print(f"Pass@k score: {sum(passk_score.values())}/{len(passk_score)}={sum(passk_score.values())/len(passk_score)}")
