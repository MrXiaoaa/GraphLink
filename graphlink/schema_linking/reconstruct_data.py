                ...
                if os.path.isdir(project_name_path):
                    for db_name in os.listdir(project_name_path):
                        ...
                        if db_name == "json":
                            ...
                            # 正确去掉 .json 后缀（strip() 会错误地去掉字符集合中的任意字符）
                            if db_name.endswith(".json"):
                                db_name_without_ext = db_name[:-5]  # 去掉 ".json"
                            else:
                                db_name_without_ext = db_name
                            
                            # 检查是否包含点号（用于分隔 project.database）
                            if '.' not in db_name_without_ext:
                                logger.warning(f"Skipping {entry}/{project_name}/{db_name}: no dot separator found")
                                continue
                            
                            folder_name = db_name_without_ext.split(".")[0]
                            file_name = '.'.join(db_name_without_ext.split(".")[1:])
                            if db_name.endswith(".json"):
                                file_name += ".json"  # 恢复 .json 后缀
                                ...
                                if schema_name == "DDL.csv":
                                    ...
                                    for i in range(len(table_name_list)):
                                        ...
                                        if reduce_col and ddl_sl_flag:
                                            ...
                                            # 检查 DDL 是否为空或只包含空的 CREATE TABLE
                                            if not col_names or col_names.strip() == "CREATE TABLE ... ();" or "();" in col_names:
                                                # DDL 为空，直接从 .json 文件读取所有列
                                                col_names = None
                                        ...
                                        for j in range(len(table_json[f"{column_prefix}names"])):
                                            ...
                                            if reduce_col and ddl_sl_flag:
                                                ...
                                                elif table_json[f"{column_prefix}names"][j] in col_names:
